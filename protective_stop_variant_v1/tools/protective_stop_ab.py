...
"""Protective-stop variant A/B harness (standalone, no edits to existing runtime code).

Purpose
- Evaluate whether adding an explicit loss-protection stop (entry-based) improves
  drawdown while preserving return.
- Compare three existing target engines under identical data/execution assumptions:
  1) runtime_switch_loop.py logic (v1)
  2) runtime_switch_loop_v2_controlplane.py logic (v2)
  3) FEV1-0001 fast-entry override logic

Design constraints
- New code path only.
- Reuses existing loaders, profile locks, and target builders.
- Does not modify existing runtime/backtest modules.
"""
...
def _simulate_intraday_with_protective_stop(
    *,
    symbols: list[str],
    ...
    """Minute-level simulation with optional entry-based protective stop.

    Protective stop rule:
    - For each held symbol, track average entry price (split-adjusted).
    - If intraday low <= entry_price * (1 - protective_stop_pct/100), flatten position.
    - Protective stop exits are blocked from same-day rebalance re-entry.
    """
    ...
    for d in aligned_days:
        ...
        # Split adjust carry inventory + cost basis so PnL continuity is correct.
        ...
        # A) Optional protective stop first (hard risk cap).
        if enable_protective_stop and protective_stop_pct > 0.0 and day_idx > 0:
            ...
            for sym in symbols:
                ...
                # Fallback only when history exists but avg_cost is unavailable.
                ...
                for ts, _o, _h, low_px, close_px in minutes:
                    if low_px <= stop_price:
                        exit_ts = ts
                        # Conservative runtime-style fill proxy.
                ...
                events.append(
                    ab.EventRecord(
                        day=d,
                        event_type="protective_stop_sell",
                        symbol=sym,
                        side="sell",
                        qty=float(held_qty),
                        price=float(exit_px),
                        ts=exit_ts,
                        reason="entry_based_protective_stop",
                        trigger_price=float(stop_price),
                        trail_stop_price=0.0,
                    )
                )

        # B) Existing profit-lock logic (same as baseline simulator), skipping already closed symbols.
        if profile.enable_profit_lock and day_idx > 0:
            for sym in symbols:
                ...
                # Runtime-like order-type emulation branch.
        ...
        # Last trade prices around rebalance time.
        ...
        # C) Rebalance at eval-time using existing order intent builder.
        if target:
            ...
            for intent in intents:
                ...
                if intent.side == "sell":
                    ...
                    # Volume-weighted average entry for protective stop reference.
                    if new_qty > 0.0:
...
def _build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        description=(
            "Standalone protective-stop A/B harness for v1, v2, and FEV1 target engines "
    ...
    # Fixed FEV1-0001 shape (explicit, reproducible).
    ...
    p.add_argument("--protective-stop-pcts", default="0,4,5,6,7,8")

    p.add_argument("--reports-dir", default=str(ROOT / "protective_stop_variant_v1" / "reports"))
    p.add_argument("--output-prefix", default="protective_stop_ab")
    return p
...
def main(argv: list[str] | None = None) -> int:
    ...
    baseline_by_engine_window: dict[tuple[str, str], dict[str, Any]] = {}
    ...
    for engine in engines:
        ...
        for stop_pct in stop_pcts:
            for window in windows:
                ...
                sim = _simulate_intraday_with_protective_stop(
                    symbols=symbols,
                    aligned_days=aligned_days,
                    price_history=price_history,
                ...
                rows.append(row)
                if abs(float(stop_pct)) <= 1e-12:
                    baseline_by_engine_window[(engine, window)] = row

    # Add baseline deltas.
    for row in rows:
        base = baseline_by_engine_window.get((str(row["engine"]), str(row["window"])))
        if base is None:
            row["delta_return_vs_base_pct"] = 0.0
            row["delta_maxdd_vs_base_pct"] = 0.0
            row["delta_equity_vs_base_usd"] = 0.0
            continue
        row["delta_return_vs_base_pct"] = float(row["return_pct"] - base["return_pct"])
        row["delta_maxdd_vs_base_pct"] = float(row["maxdd_pct"] - base["maxdd_pct"])
        row["delta_equity_vs_base_usd"] = float(row["final_equity"] - base["final_equity"])

    # Rank only non-zero stop candidates by engine using a simple risk-return score.
    ...
    for r in rows:
        ...
        agg["windows"] += 1
        agg["sum_delta_return"] += float(r["delta_return_vs_base_pct"])
        agg["sum_delta_maxdd"] += float(r["delta_maxdd_vs_base_pct"])
        agg["sum_delta_equity"] += float(r["delta_equity_vs_base_usd"])

    for _k, agg in by_key.items():
        n = max(1, int(agg["windows"]))
        avg_ret = float(agg["sum_delta_return"]) / n
        avg_dd = float(agg["sum_delta_maxdd"]) / n
        avg_eq = float(agg["sum_delta_equity"]) / n
        score = avg_ret + (avg_eq / 1000.0) - (0.7 * avg_dd)
        ranked.append(
            {
                **agg,
    ...
    # Human-friendly best row per engine+window.
    best_per_engine_window: dict[tuple[str, str], dict[str, Any]] = {}
    for r in rows:
        key = (str(r["engine"]), str(r["window"]))
        prev = best_per_engine_window.get(key)
        # Primary objective: max return. tie-break: lower maxdd.
        if prev is None:
            best_per_engine_window[key] = r
            continue
        if float(r["return_pct"]) > float(prev["return_pct"]):
            best_per_engine_window[key] = r
        elif float(r["return_pct"]) == float(prev["return_pct"]) and float(r["maxdd_pct"]) < float(prev["maxdd_pct"]):
            best_per_engine_window[key] = r
    ...
    summary = {
        ...
        "best_per_engine_window": [best_per_engine_window[k] for k in sorted(best_per_engine_window.keys())],
    }
