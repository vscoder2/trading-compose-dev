...
from composer_original.experiment.aggr_v2.model_types import OverlayConfig as V2OverlayConfig
from composer_original.experiment.aggr_v2.overlays import apply_inverse_blocker
...
def _sma(values: list[float], window: int) -> float | None:
    if len(values) < window:
        return None
    return float(sum(values[-window:]) / float(window))
...
def _rv20_ann(values: list[float]) -> float:
    ...
    return float((var**0.5) * (252.0**0.5))
...
def _load_regime_state(store: StateStore) -> RegimeState:
    raw = store.get("switch_regime_state", None)
    if not isinstance(raw, dict):
        return RegimeState()
    state = RegimeState()
    for key in ("current_variant", "cond2_streak", "cond3_streak", "cond2_false_streak", "forced_baseline_days", "high_vol_lock"):
        if key in raw:
            setattr(state, key, raw[key])
    if state.current_variant not in {"baseline", "inverse_ma20", "inverse_ma60"}:
        state.current_variant = "baseline"
    return state


def _variant_overlay(variant: str) -> V2OverlayConfig:
    if variant == "inverse_ma20":
        return V2OverlayConfig(enable_inverse_blocker=True, trend_symbol="SOXL", trend_ma_days=20)
    if variant == "inverse_ma60":
        return V2OverlayConfig(enable_inverse_blocker=True, trend_symbol="SOXL", trend_ma_days=60)
    return V2OverlayConfig(enable_inverse_blocker=False, trend_symbol="SOXL", trend_ma_days=60)


def _apply_variant_to_target(
    target_weights: dict[str, float],
    daily_closes: dict[str, list[float]],
    variant: str,
) -> tuple[dict[str, float], str]:
    overlay = _variant_overlay(variant)
    adjusted, note = apply_inverse_blocker(
        proposed_weights=dict(target_weights),
        close_history=daily_closes,
        overlay=overlay,
    )
...
def _run_loop(args: argparse.Namespace) -> int:
    ...
    logger.info(
        (
            "Starting switch runtime mode=%s profile=%s execute_orders=%s eval_time=%s "
    ...
    while True:
        ...
        # Intraday profit-lock checks before main daily eval time.
        if (
        ...
        if len(soxl_hist) < 20:
            ...
            regime_state = _load_regime_state(store)
            metrics = _compute_regime_metrics(soxl_hist)
            prev_variant = regime_state.current_variant
            variant, variant_reason = _choose_variant(metrics, regime_state)
        ...
        switched_target, inv_note = _apply_variant_to_target(
            baseline_target,
            daily_closes,
            variant,
        )
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Standalone switch-profile runtime loop (paper/live) in separate folder. "
            "Implements dynamic variant selection: baseline / inverse_ma20 / inverse_ma60."
        )
    )
    ...
    parser.add_argument("--strategy-profile", choices=sorted(PROFILES.keys()), default="aggr_adapt_t10_tr2_rv14_b85_m8_M30_intraday_pl_5m")
...
def main() -> int:
    ...
    try:
        ...
        logger.exception("Switch runtime loop failed")
        return 1
