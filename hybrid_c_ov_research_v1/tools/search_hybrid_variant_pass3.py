...
def _build_candidates() -> list[CandidateConfig]:
    # Pass-3 targeted family:
    # Keep profile close to C baseline and expand overlay behavior to include
    # dd-only and soft-shock variants to recover short windows while protecting 2Y.
    profile_knobs = [
    ...
    overlay_knobs = [
        # dd-only family: no shock trigger, brake on drawdown only
        (0.0, 0, 8.0, 20, 1),
        (0.0, 0, 8.0, 20, 2),
        (0.0, 0, 12.0, 20, 1),
        (0.0, 0, 12.0, 20, 2),
...
def main() -> int:
    ...
    report_dir = ROOT / "hybrid_c_ov_research_v1" / "reports_pass3"
    ...
    # Pass-1 sweep: maximize short-window closeness to C, with hard preference
    # for 2Y >= OV via heavy penalty when violated.
    ...
    for idx, cc in enumerate(candidates, start=1):
        ...
        score = (short_excess * 0.15) - (short_penalty * 1.00) - (long_penalty * 3.00)
    ...
    # Pass-2 validation: top 24 from pass-1, rerun.
    top20 = pass1_df.head(20).copy()
    pass2_rows: list[dict[str, Any]] = []
    topN = 24
    topk = pass1_df.head(topN).copy()
    for idx2, (_, r) in enumerate(topk.iterrows(), start=1):
        cc = CandidateConfig(
        ...
        print(f"[pass2] {idx2}/{len(topk)} done")
    ...
    pass2_csv = report_dir / "pass2_top_validated.csv"
    ...
    # nearest candidates by objective: minimize short penalty among those passing 2Y gate
    pass2_with_gate = pass2_df.copy()
    pass2_with_gate["gate_ok_2y_vs_ov"] = pass2_with_gate["2y_ret"] >= float(ov_metrics["2y"]["return_pct"])
    gate_ok_df = pass2_with_gate[pass2_with_gate["gate_ok_2y_vs_ov"]].sort_values(
        ["short_penalty_sum", "2y_ret"], ascending=[True, False]
    )
    nearest_gate_candidate = gate_ok_df.iloc[0].to_dict() if not gate_ok_df.empty else None
    ...
    out = {
        ...
        "nearest_gate_candidate": nearest_gate_candidate,
        "reports": {
