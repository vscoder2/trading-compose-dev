...
from meta_router_v2.tools.historical_meta_router_v2_windows import (
    RouterV2Config,
    _build_meta_v2_targets_and_thresholds,
    _parse_hhmm,
)
...
def _score_candidate_row(candidate: dict[str, Any], windows: list[str]) -> dict[str, Any]:
    ...
    # Strongly prioritize wins, then uplift, then DD overage.
    candidate["ranking_score"] = float((wins * 2000.0) + (avg_uplift * 20.0) - (avg_dd_over * 5.0))
    return candidate


def _evaluate_candidate(task: tuple[int, RouterV2Config, list[str], date]) -> dict[str, Any]:
    ...
    meta_targets, meta_thresholds, selected_engine, _selected_reason = _build_meta_v2_targets_and_thresholds(
        ...
        close_map_by_symbol=G["close_map_by_symbol"],
...
def _build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description="Parallel parameter sweep for meta_router_v2")
    ...
    p.add_argument("--coarse-windows", default="1m,2m,6m,1y")
    p.add_argument("--full-windows", default="1m,2m,3m,4m,5m,6m,1y,2y,3y,4y,5y,7y")

    # Sweep dimensions.
    p.add_argument("--grid-lookback-days", default="10,15,20,30")
    p.add_argument("--grid-vol-penalty", default="0.8,1.2")
    p.add_argument("--grid-dd-penalty", default="0.08,0.16")
    p.add_argument("--grid-switch-penalty", default="0.0,0.05")
    p.add_argument("--grid-min-edge-to-switch", default="0.0,0.05")
    p.add_argument("--grid-min-hold-days", default="1,2")
    p.add_argument("--grid-burst-v1-bonus", default="0.10,0.25")
    p.add_argument("--grid-riskoff-m0106-bonus", default="0.10,0.25")

    # Keep regime thresholds configurable as narrow/fixed anchors.
    ...
    p.add_argument("--coarse-workers", type=int, default=24)
    p.add_argument("--full-workers", type=int, default=12)
    p.add_argument("--topk-to-full", type=int, default=24)

    p.add_argument("--reports-dir", default=str(ROOT / "meta_router_v2" / "reports"))
    p.add_argument("--output-prefix", default="sweep_meta_router_v2")
    return p
...
def main(argv: list[str] | None = None) -> int:
    ...
    for values in product(
        _parse_csv_ints(args.grid_lookback_days),
        _parse_csv_floats(args.grid_vol_penalty),
        _parse_csv_floats(args.grid_dd_penalty),
        _parse_csv_floats(args.grid_switch_penalty),
        _parse_csv_floats(args.grid_min_edge_to_switch),
        _parse_csv_ints(args.grid_min_hold_days),
        _parse_csv_floats(args.grid_burst_v1_bonus),
        _parse_csv_floats(args.grid_riskoff_m0106_bonus),
    ):
        cfg = RouterV2Config(
    ...
    for w in full_windows:
        ...
        s_v1 = _simulate_window(target_by_day=v1_targets, threshold_by_day=v1_thresholds, start_day=start_day, end_day=end_day, bundle=local_bundle)
        s_v2 = _simulate_window(target_by_day=v2_targets, threshold_by_day=v2_thresholds, start_day=start_day, end_day=end_day, bundle=local_bundle)
        s_m0 = _simulate_window(target_by_day=m0106_targets, threshold_by_day=m0106_thresholds, start_day=start_day, end_day=end_day, bundle=local_bundle)
        baseline[w] = {
    ...
    coarse_results.sort(
        key=lambda r: (r["ranking_score"], r["wins_vs_best_baseline"], r["avg_uplift_pct_vs_best_baseline"]),
        reverse=True,
    )
    ...
    for idx, row in enumerate(full_candidates, start=1):
        cfg = RouterV2Config(
            ...
            vol_penalty=float(row["vol_penalty"]),
            dd_penalty=float(row["dd_penalty"]),
            switch_penalty=float(row["switch_penalty"]),
            min_edge_to_switch=float(row["min_edge_to_switch"]),
            ...
            burst_v1_bonus=float(row["burst_v1_bonus"]),
            ...
            riskoff_m0106_bonus=float(row["riskoff_m0106_bonus"]),
        )
